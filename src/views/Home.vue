<template>
  <div class="home">
    <h1 class="ml10">
      <span
        v-for="(firstStr, index) in firstStrs"
        :key="index"
        class="text-wrapper"
      >
        <span class="letter">{{ firstStr }}</span>
      </span>
      <br class="sp-br" />
      <span
        v-for="(secondStr, index) in secondStrs"
        :key="index"
        class="text-wrapper"
      >
        <span class="letter">{{ secondStr }}</span>
      </span>
    </h1>
...
export default defineComponent({
  ...
  components: {},
  setup() {
    let firstStr = 'Drawing '
    let firstStrs = firstStr.split('')

    let secondStr = 'Game'
    let secondStrs = secondStr.split('')
